...
def deactivateProduct(naam, enddate):
    'Takes a name and a date. This function deactivates the product, meaning it'
...
def alterPrice(name, newprice):
    'Takes a name and a new price, then alters the products price in the database'
...
cursor.execute('''SELECT name, value, price.datetimeStart, price.datetimeEnd FROM Product INNER JOIN price ON Product.productId = price.productId''')
print('{:30} {:10} {:15} {:10}'.format('Productnaam', 'Prijs', 'Start datum', 'Eind datum'))
for row in cursor:
    print('{:30} €{:<9.2f} {:<15} {:<10}'.format(row[0], row[1], row[2], str(row[3])))
